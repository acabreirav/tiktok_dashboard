...
- ***⚠️ Primera corrida con el fix (workflow 2026-07-09): job en verde pero INCOMPLETA.** El snapshot TikTok trajo **31 de 49 cuentas** y **no se generó snapshot de Instagram**. **Causa confirmada por Álvaro: se agotó el crédito Apify** (no rate-limit) — el run se quedó sin saldo a mitad y aun así cerró SUCCEEDED con datos parciales.)*

*(2026-07-13, guardas anti-hueco en `src/fetch.py` (a raíz del 07-09; NO se tocó la lógica async+polling+runs independientes):*
- ***Guarda de completitud:** cada run registra en el log `cuentas pedidas: N / traídas: M`. Si TikTok trae menos de las pedidas, emite `::error::` (anotación roja en el run) + aviso en `$GITHUB_STEP_SUMMARY` y **falla el job** (exit≠0) → el workflow NO commitea el snapshot cojo en verde. IG incompleto solo avisa (`::warning::`), no tumba el job (es complementaria).*
- ***Detección de 402:** `apify_client.py` distingue `ApifyCreditError` (HTTP 402 / not-enough-usage-to-run-paid-actor) de un fallo genérico; el log dice claramente "sin crédito Apify — corrida omitida", no un bug del actor.*
- ***29 tests en verde** (nuevo: 402 → ApifyCreditError). Guarda de completitud verificada con mock (2/3 → exit 1 + resumen). Motivo de fondo: el histórico es el activo; un snapshot parcial es un hueco irrecuperable en la serie.)*
